...
class labelme2coco(object):
    ...
    def categorie(self,label):
        categorie={}
        categorie['supercategory'] = 'facade'       
        if self.only_labels is not None:
            categorie['id'] = self.only_labels[label]
        else:
            categorie['id'] = len(self.label)+1 #len(self.label)+1 # 0 默认为背景
        categorie['name'] = label #label[1]
        return categorie

    def annotation(self,points,label,num):
        annotation={}
        annotation['segmentation']= [[p for point in points for p in point]]
